...
class Mapper4Javascript extends UtilsBase4Javascript {
	...
	validateAlreadyRegisteredMapper(key, objectTypeName, keyName) {
		...
		if (!!key && !!this.mapperTypes[key] && this.mapperTypes[key].typeName !== objectTypeName) {
			throw new Error("EXCEPTION [validateAlreadyRegisteredMapper]: the " + keyName + ' "' + key + '" has been already registered.\n' + this.getCallStack());
		}
	}
	...
	validateAlreadyRegisteredMapperHasNotBeenRegistered(key, keyName, mappers) {
		...
		if (!!key && !mappers[key]) {
			throw new Error(
				"EXCEPTION [validateAlreadyRegisteredMapperHasNotBeenRegistered]: the " +
					keyName +
					' "' +
					key +
	...
	validateOriginObjectMustBeAnInstancedObject(originObject) {
		if (!originObject) {
			throw new Error(
				'EXCEPTION [validateOriginObjectMustBeAnInstancedObject]: the "originObject" must be an instance of some Object.\n' + this.getCallStack()
			);
		}
	}
	...
	validateObjectMustBeRegistered(obj, mapper) {
		if (!!obj && !mapper) {
			throw new Error(
				'EXCEPTION [validateObjectMustBeRegistered]: the "' + obj.__proto__.constructor.name + '" must be registered before to use it.\n' + this.getCallStack()
			);
		}
	}
	...
	validateDestinationObjectTypeMustBeRegistered(destinationObjectType, destinationMapper) {
		if (!destinationMapper) {
			throw new Error(
				'EXCEPTION [validateDestinationObjectTypeMustBeRegistered]: the "' +
					destinationObjectType.toString() +
					'" must be registered before to use it.\n".\n' +
					this.getCallStack()
			);
		}
	}
	...
	validateDestinationObjectType(destinationObjectType, destinationObject, destinationMapper) {
		if (!!destinationObjectType && !!destinationObject && destinationObject.__proto__.constructor.name !== destinationMapper.typeName) {
			throw new TypeError(
				'EXCEPTION [validateDestinationObjectType]: The destination object type expeted is "' +
					destinationObjectType +
					'" but the recived object type was "' +
	...
	doMappedObjectForList(getterMapperConfiguration) {
		...
		getterMapperConfiguration.originObjects.forEach(
			function (originObject, index, allOriginObjects) {
	...
	mustMapAttribute(getterMapperConfiguration, attributeName) {
		...
		if ((ignoreAllAttributes && exceptedAttributes.indexOf(attributeName) < 0) || ignoredAttributes.indexOf(attributeName) >= 0) {
			return false;
		}
	...
	copyAttribute(getterMapperConfiguration, originObject, key) {
		if (
			!!getterMapperConfiguration.destinationObject[key] &&
	...
	mapAllOriginAttributesCallback(getterMapperConfiguration, originObject) {
		...
		originObjectKeys.forEach(
			function (key, index, allKeys) {
				if (!this.isFunction(originObject[key]) && this.mustMapAttribute(getterMapperConfiguration, key)) {
					getterMapperConfiguration.destinationObject[key] = originObject[key];
				}
			}.bind(this)
		);
	...
	mapAllCallback(getterMapperConfiguration, originObject) {
		...
		destinationObjectKeys.forEach(
			function (key, index, allKeys) {
				if (!!originObject[key] && this.mustMapAttribute(getterMapperConfiguration, key)) {
					getterMapperConfiguration.destinationObject[key] = originObject[key];
				}
			}.bind(this)
		);
	...
	mapAllMethodsCallback(getterMapperConfiguration, originObject) {
		...
		destinationObjectKeys.forEach(
			function (key, index, allKeys) {
				if (key !== "constructor" && !!originObject[key] && this.isFunction(originObject[key]) && this.mustMapAttribute(getterMapperConfiguration, key)) {
	...
	getAllMappedObjectKeys(mappedObject) {
		...
		if (!!mappedObject.__proto__) {
			keys = keys.concat(Object.getOwnPropertyNames(mappedObject.__proto__));
		}
	...
	registerMapper(registerMapperConfiguration) {
		...
		let destinationKey =
			registerMapperConfiguration.destinationKey ||
		...
		if (!this.mapperTypes[originKey]) {
			this.registerMepperKeyTypes(registerMapperConfiguration.originObjectType || this.defaultConfiguration.originObjectType, originKey);
		}

		if (!this.mapperTypes[destinationKey]) {
			this.registerMepperKeyTypes(registerMapperConfiguration.destinationObjectType || this.defaultConfiguration.destinationObjectType, destinationKey);
		}
