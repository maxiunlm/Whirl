...
describe("IoC4Javascript - ", () => {
	...
	describe("getInstanceOf - ", () => {
		...
		it("with a Definition Callback invokes instanceDefinitionCallback method", () => {
			let sut = new IoC4Javascript();
			let myFifthClass = new apiFakes.MyFifthClass();
			spyOn(apiFakes.MyFifthClass.prototype, "fifthClassMethod").and.callThrough();

			let result = sut.getInstanceOf(apiFakes.constructorKey, myFifthClass.fifthClassMethod);

			expect(apiFakes.MyFifthClass.prototype.fifthClassMethod.calls.count()).toEqual(apiFakes.once);
		});
	});
});
