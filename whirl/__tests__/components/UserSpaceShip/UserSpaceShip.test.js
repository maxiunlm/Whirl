...
describe("UserSpaceShip - ", () => {
	...
	describe("startShotting - ", () => {
		it('With the "shot configuration" string key invokes the "setState" method from "sut" object', () => {
			...
			spyOn(sut, "canShotAgain").and.callFake(() => true);
		...
		it('With the "state data" sets the "state.shot" values', () => {
			...
			expect(sut.shots[commonFakes.firstIndex].left).toEqual(commonFakes.position.left);
			expect(sut.shots[commonFakes.firstIndex].top).toEqual(commonFakes.position.top);
			expect(sut.shots[commonFakes.firstIndex].angle).toEqual(commonFakes.startAngle);
